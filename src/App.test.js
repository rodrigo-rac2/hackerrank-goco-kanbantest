...
test('For each task in stage 0 and stage 1, backward and forward buttons are correctly enabled/disabled', () => {
    ...
    expect(getByTestId('task-1-back').hasAttribute('disabled')).toEqual(false); // makes no sense to go back from task 1
    expect(getByTestId('task-1-forward').hasAttribute('disabled')).toBeFalsy();

    expect(getByTestId('task-2-name').innerHTML).toBe('task 2');
    expect(getByTestId('task-2-back').hasAttribute('disabled')).toEqual(false);
    expect(getByTestId('task-2-forward').hasAttribute('disabled')).toBeFalsy();

    expect(getByTestId('task-3-name').innerHTML).toBe('task 3');
    expect(getByTestId('task-3-back').hasAttribute('disabled')).toEqual(false);
...
test('For each task in stage 2 and stage 3, backward and forward buttons are correctly enabled/disabled', () => {
	...
	expect(getByTestId('task-9-forward').hasAttribute('disabled')).toEqual(false); // there are more tasks, so it should be possible to move forward

	expect(getByTestId('task-10-name').innerHTML).toBe('task 10');
	expect(getByTestId('task-10-back').hasAttribute('disabled')).toBeFalsy();
	expect(getByTestId('task-10-forward').hasAttribute('disabled')).toEqual(false);
});
...
test('For a task in stage 0, can be moved forward till stage 4 and buttons are correctly enabled and disabled in stage 4', () => {
    ...
    expect(backlogStage.contains(getByTestId(taskId))).toBe(true); // this should be true, the backlog already has tasks
    expect(toDoStage.contains(getByTestId(taskId))).toBe(false);
    expect(onGoingStage.contains(getByTestId(taskId))).toBe(false);
    expect(doneStage.contains(getByTestId(taskId))).toBe(false);
    ...
    expect(getByTestId('task-1-forward').hasAttribute('disabled')).toEqual(false);
});
...
test('For a task in stage 4, can be moved backward till stage 0 and check for buttons are enabled/disabled correctly', () => {
    ...
    expect(backlogStage.contains(getByTestId(taskId))).toBe(false);
    expect(toDoStage.contains(getByTestId(taskId))).toBe(false);
    expect(onGoingStage.contains(getByTestId(taskId))).toBe(false); // there are no tasks at all in this scope
    expect(doneStage.contains(getByTestId(taskId))).toBe(true);
    ...
    expect(getByTestId('task-9-back').hasAttribute('disabled')).toEqual(false);
    expect(getByTestId('task-1-forward').hasAttribute('disabled')).toBeFalsy();
...
test('after many forward and back operations, tasks are in correct state', () => {
	...
	expect(backlogStage.children.length).toBe(3); // tasks 0, 1, 2 and 3 from above
    expect(backlogStage.contains(getByTestId('task-3-name'))).toBe(true);
    expect(backlogStage.contains(getByTestId('task-6-name'))).toBe(false);

    expect(toDoStage.children.length).toBe(3);
    expect(toDoStage.contains(getByTestId('task-1-name'))).toBe(false);
    expect(toDoStage.contains(getByTestId('task-2-name'))).toBe(false);
    expect(toDoStage.contains(getByTestId('task-8-name'))).toBe(false);

    expect(onGoingStage.children.length).toBe(2);
    expect(onGoingStage.contains(getByTestId('task-4-name'))).toBe(false);
    expect(onGoingStage.contains(getByTestId('task-5-name'))).toBe(false);
    expect(onGoingStage.contains(getByTestId('task-9-name'))).toBe(false);
    expect(onGoingStage.contains(getByTestId('task-10-name'))).toBe(false);

    expect(doneStage.children.length).toBe(2);
    expect(doneStage.contains(getByTestId('task-7-name'))).toBe(false);
});